...
import { AdminWalletListStore } from './admin-wallet-list.store'

@Component({
  template: `
    <ng-container *ngIf="vm$ | async as vm">
      <ui-page-header title="Wallets" linkPath="create" linkTitle="Create Wallet"></ui-page-header>
      <ng-container *ngIf="vm.loading">
        <div class="flex py-36 animate-pulse justify-center align-center">LOADING...</div>
      </ng-container>
      <ng-container *ngIf="!vm.loading">
        <ng-container *ngIf="!vm.items?.length">
          <div class="flex py-20 justify-center align-center">No Wallets found...</div>
        </ng-container>
  ...
  `,
  providers: [AdminWalletListStore],
})
export class AdminWalletListComponent implements OnInit {
  readonly vm$ = this.store.vm$
  constructor(private readonly store: AdminWalletListStore) {}

  ngOnInit(): void {
    this.store.loadWalletsEffect()
  }
}
